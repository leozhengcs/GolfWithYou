<footer class="bg-sky-background w-full">
	<!-- <div class="relative bottom-0 z-10 max-w-[100%]">
		<img src="/images/croppedgreens.png" class="" alt="" />
		<img
			src="/images/croppedflag.png"
			class="absolute bottom-1/4 left-3/5 z-20 max-w-[20%]"
			alt=""
		/>
	</div> -->
	<div class="relative z-20 justify-between bg-[#354F52] p-5">
		<div class="flex justify-between">
			<div class="relative z-20">
				<span class="text-xs text-white">© Teesaway 2025. All rights reserved</span>
			</div>
			<div class="relative z-10 flex gap-5">
				<a
					href="/files/terms-of-use.pdf"
					target="_blank"
					rel="noopener"
					class="text-secondary-shade text-xs underline md:text-sm">Terms of Use</a
				>
				<a
					href="/files/privacy-policy.pdf"
					target="_blank"
					rel="noopener"
					class="text-secondary-shade text-xs underline md:text-sm">Privacy Policy</a
				>
			</div>
		</div>
	</div>
</footer>
